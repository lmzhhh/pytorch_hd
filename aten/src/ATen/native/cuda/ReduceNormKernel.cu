...
void norm_kernel_cuda_impl(TensorIterator& iter, Scalar val) {
  ...
  if (p == static_cast<double>(0)) {
    if (! at::globalContext().heterogeneousDeterministicAlgorithms())
      ...
      gpu_reduce_kernel_heterogeneous_deterministic<scalar_t, out_t>(iter, NormZeroOps<scalar_t, acc_t>(), 0);
  } else if (p == static_cast<double>(1)) {
    if (! at::globalContext().heterogeneousDeterministicAlgorithms())
      gpu_reduce_kernel<scalar_t, out_t>(iter, NormOneOps<scalar_t, acc_t>(), 0);
    else
      gpu_reduce_kernel_heterogeneous_deterministic<scalar_t, out_t>(iter, NormOneOps<scalar_t, acc_t>(), 0);
  } else if (p == static_cast<double>(2)) {
    if (! at::globalContext().heterogeneousDeterministicAlgorithms())
      gpu_reduce_kernel<scalar_t, out_t>(iter, NormTwoOps<scalar_t, acc_t>(), 0);
    else
      gpu_reduce_kernel_heterogeneous_deterministic<scalar_t, out_t>(iter, NormTwoOps<scalar_t, acc_t>(), 0);
  } else if (p == static_cast<double>(INFINITY)) {
    if (! at::globalContext().heterogeneousDeterministicAlgorithms())
      gpu_reduce_kernel<scalar_t, out_t>(iter, AbsMaxOps<scalar_t, acc_t>(), 0);
    else
      gpu_reduce_kernel_heterogeneous_deterministic<scalar_t, out_t>(iter, AbsMaxOps<scalar_t, acc_t>(), 0);
  } else if (p == static_cast<double>(-INFINITY)) {
    if (! at::globalContext().heterogeneousDeterministicAlgorithms())
      gpu_reduce_kernel<scalar_t, out_t>(iter, AbsMinOps<scalar_t, acc_t>(), std::numeric_limits<acc_t>::max());
    else
      gpu_reduce_kernel_heterogeneous_deterministic<scalar_t, out_t>(iter, AbsMinOps<scalar_t, acc_t>(), std::numeric_limits<acc_t>::max());
  } else {
    if (! at::globalContext().heterogeneousDeterministicAlgorithms())
      gpu_reduce_kernel<scalar_t, out_t>(iter, NormOps<scalar_t, acc_t>{ acc_t(p) }, 0);
    else
      gpu_reduce_kernel_heterogeneous_deterministic<scalar_t, out_t>(iter, NormOps<scalar_t, acc_t>{ acc_t(p) }, 0);
  }
}
